...
def get_settings(db:DbSession) -> SettingsDTO:
    return get_settings_service(db)


@router.put("", response_model=SettingsDTO)
def update_settings(request: UpdateSettingsRequest, db:DbSession) -> SettingsDTO:
    return update_settings_service(request,db)
